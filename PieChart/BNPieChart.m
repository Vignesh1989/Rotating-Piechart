...
@synthesize slicePortions,sliceRadianRangeArray,pieDelegate;
...
+ (BNPieChart *)pieChartSampleWithFrame:(CGRect)frame  ForData:(NSArray *)values WithLabels:(NSArray *)labels {
	BNPieChart *chart = [[[BNPieChart alloc]
						  initWithFrame:frame  ForData:values WithLabels:labels ] autorelease];
...
- (id)initWithFrame:(CGRect)frame  ForData:(NSArray *)values WithLabels:(NSArray *)labels {
	if (self = [super initWithFrame:frame]) {
		...
		sliceRadianRangeArray=[[NSMutableArray alloc] init];
		...
		for (NSNumber *value in values)
		{
			[self addSlicePortion:([value floatValue]/sum) withName:[labels objectAtIndex:i]];
			angle+=([value floatValue]/sum)*360;
			NSNumber *angleNumber=[NSNumber numberWithFloat:angle];
			[sliceRadianRangeArray insertObject:angleNumber  atIndex:i];
			i++;
		}
		[values release];
		[labels release];
...
- (void)drawRect:(CGRect)rect {
	...
	CGContextAddArc(context, centerX, centerY ,
					(radius-10.0), 0, 2*M_PI, 1);
	CGContextClip(context);
	
	// Set up the gradient for the glare.
	size_t num_locations = 2;
	CGFloat locations[2] = {0.0, 1.0};
	CGFloat components[8] = {1.0, 1.0, 1.0, 0.25, 1.0, 1.0, 1.0, 0.25};
	...
	CGContextDrawLinearGradient(context, gradient,
								...
								CGPointMake(centerX - radius, centerY - radius), 1);
...
- (void)drawSlice:(int)index inContext:(CGContextRef)context {
	...
	UIColor* darken = [UIColor colorWithWhite:0.0 alpha:1.0];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    //Get the touch co-ordinate
	UITouch *touch = [touches anyObject];
	CGPoint touchPoint = [touch locationInView:self];
	
	
	if(touchPoint.y-centerY <radius && touchPoint.x-centerX <radius)
	{
		//Define positive x vector starts with origin and normalise
		CGPoint vector1=CGPointMake(radius,0);
	    double length=sqrt(vector1.x*vector1.x+vector1.y*vector1.y);
		vector1.x=vector1.x/length;
		vector1.y=vector1.y/length;
		
		//Define origin to touch point vector and normalise	
		CGPoint vector2=CGPointMake(touchPoint.x-centerX, touchPoint.y-centerY);
		length=sqrt(vector2.x*vector2.x+vector2.y*vector2.y);
		vector2.x=vector2.x/length;
		vector2.y=vector2.y/length;
		
		//Angle between vectors	
	    double angle=acos(vector1.y*(vector2.y)+vector1.x*(vector2.x));
		angle=angle*180/(M_PI);
		
		//Bring negative angle in 360 degree scale	
		if(touchPoint.x> centerX && touchPoint.y > centerY)
		{
			angle=360-angle;
		}
		if(touchPoint.x< centerX && touchPoint.y > centerY)
		...
		//Pie defined in anti clockwise direction
		...
		NSInteger i = 0;
		for(NSNumber *value in sliceRadianRangeArray)
		{
			
			if(angle < [value floatValue])
			{
				[[self pieDelegate] pieViewTouchedAtIndex:i];
				break;
			}
			i++;
		}
